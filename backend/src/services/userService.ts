...
const syncConfigs = {
  camions: {
    ...
    mapData: (camion: Camion) => {
      logger.debug(`Raw camion data for ID ${camion.id}: ${JSON.stringify(camion)}`);

      const nom = typeof camion.nom === 'string' && camion.nom.trim() ? camion.nom.trim() : `Camion_${camion.id || uuidv4()}`;
      const type = typeof camion.type === 'string' && camion.type.trim() ? camion.type.trim() : 'Transport';

      if (nom === `Camion_${camion.id || uuidv4()}`) {
        logger.warn(`Missing or invalid nom for camion ${camion.id}. Using fallback '${nom}'.`);
      }
      if (type === 'Transport') {
        logger.warn(`Missing or invalid type for camion ${camion.id}. Using fallback 'Transport'.`);
      }
  ...
  users: {
    ...
    mapData: (user: User) => {
      if (!user.nom)
        logger.warn(`Missing nom for user ${user.id}. Using fallback 'unknown'.`);
      if (!user.email)
        logger.warn(`Missing email for user ${user.id}. Using fallback 'unknown'.`);
      if (!user.role)
        logger.warn(`Missing role for user ${user.id}. Using fallback 'unknown'.`);
...
const processItem = async <T extends SyncItem>(
  ...
  if (!(await shouldSyncItem(item, Model, itemId))) {
    logger.debug(`Skipping sync for ${logContext} ID ${itemId}: already synced or invalid sync_status`);
    return;
  }
  ...
  if (Model === CamionModel) {
    ...
    if (!nom || !type || typeof nom !== 'string' || typeof type !== 'string' || !nom.trim() || !type.trim()) {
      logger.error(`Invalid camion data for ID ${itemId}: nom or type missing, invalid, or empty. Data: ${JSON.stringify(item)}. Skipping.`);
      return;
    }
  }
  ...
  if (Model === CamionModel) {
    if (!mappedData.nom || !mappedData.type || typeof mappedData.nom !== 'string' || typeof mappedData.type !== 'string' || !mappedData.nom.trim() || !mappedData.type.trim()) {
      logger.error(`Mapped data for camion ID ${itemId} has invalid nom or type: ${JSON.stringify(mappedData)}. Skipping.`);
      return;
    }
  }
  ...
  try {
    ...
    logger.error(`Failed to upsert ${logContext} ID ${itemId}: ${error.message}`);
    if (error.name === 'SequelizeValidationError') {
      const messages = error.errors.map((err: any) => err.message).join(', ');
      logger.error(`Validation error for ${logContext} ID ${itemId}: ${messages}`);
    }
  }
};
...
const syncData = async <T extends SyncItem>(
  ...
  try {
    ...
    logger.error(`Erreur lors de la synchronisation de ${refPath}: ${error.message}`);
    throw error;
  }
};
