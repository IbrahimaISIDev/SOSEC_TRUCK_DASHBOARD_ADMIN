...
interface DashboardData {
  financialData: { totalRevenue: number; totalExpenses: number; netProfit: number } | null;
  notifications: { id: string; document: string; daysLeft: number; status: 'warning' | 'info' }[];
  recentDocuments: { id: string; name: string; type: string; dateAdded: string }[];
  driverActivities: { id: string; driver: string; ticket: string; date: string }[];
...
const staticDashboardData: DashboardData = {
  financialData: { totalRevenue: 125000, totalExpenses: 75000, netProfit: 50000 },
  notifications: [
    { id: '1', document: 'Permis de conduire', daysLeft: 5, status: 'warning' },
    { id: '2', document: 'Assurance camion', daysLeft: 10, status: 'warning' },
  ],
  recentDocuments: [
    { id: '1', name: 'Permis de conduire', type: 'License', dateAdded: '2025-05-01' },
    { id: '2', name: 'Assurance', type: 'Insurance', dateAdded: '2025-04-28' },
    { id: '3', name: 'Ticket #250395', type: 'Ticket', dateAdded: '2025-04-25' },
  ],
  driverActivities: [
...
export default function Dashboard() {
  const { user } = useAuth();
  const [dashboardData, setDashboardData] = useState<DashboardData>(staticDashboardData);
  ...
  return (
    ...
    >
      ...
      <Typography variant="body1" color="text.secondary" gutterBottom>
        Aperçu global des opérations de Saraya Tech
      </Typography>
        ...
        <Grid {...gridItemProps} xs={12} sm={6} md={4}>
          <DashboardCard title="Résumé Financier">
            <Typography color="text.secondary">
              Revenus totaux :{' '}
              {dashboardData.financialData?.totalRevenue.toLocaleString('fr-FR')} FCFA
            </Typography>
            <Typography color="text.secondary">
              Dépenses totales :{' '}
              {dashboardData.financialData?.totalExpenses.toLocaleString('fr-FR')} FCFA
            </Typography>
            <Typography
              color={
                dashboardData.financialData?.netProfit !== undefined && dashboardData.financialData?.netProfit >= 0
                  ? 'success.main'
                  : 'error.main'
              }
            >
              Bénéfice net :{' '}
              {dashboardData.financialData?.netProfit.toLocaleString('fr-FR')} FCFA
            </Typography>
          </DashboardCard>
        </Grid>
            ...
            <List dense aria-label="Notifications urgentes">
              {dashboardData.notifications.map((notif) => (
                <ListItem key={notif.id}>
                  <ListItemText
                    primary={notif.document}
                    secondary={`${notif.daysLeft} jours restants`}
                  />
                  <Chip
                    label={notif.status === 'warning' ? 'Urgent' : 'Info'}
                    color={notif.status === 'warning' ? 'warning' : 'info'}
                    size="small"
                    aria-label={`Statut : ${notif.status === 'warning' ? 'Urgent' : 'Info'}`}
                  />
                </ListItem>
              ))}
            </List>
        ...
        <Grid {...gridItemProps} xs={12} md={6}>
          <DashboardCard title="Revenus vs Dépenses (2025)">
            <Box sx={{ overflowX: 'auto' }}>
              <BarChart
                xAxis={[{ scaleType: 'band', data: dashboardData.revenueExpenseData.map((d) => d.month) }]}
            ...
            <List aria-label="Activités récentes des chauffeurs">
              {dashboardData.driverActivities.map((activity, index) => (
                <Box key={activity.id}>
                  ...
                  {index < dashboardData.driverActivities.length - 1 && <Divider />}
                </Box>
              ))}
            </List>
          </DashboardCard>
